...
// получаем все пропсы в объект props
...
// Используй props.modelValue, а не просто modelValue
...
const isSelected = (option) => {
  if (Array.isArray(internalValue.value)) {
    return internalValue.value.includes(option)
  } else {
...
const displayValue = computed(() => {
  if (props.multiple) {
    return internalValue.value && internalValue.value.length
